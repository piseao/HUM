...
이 설정은 자동 테스트의 대기시간만 바꿉니다. 기본 로컬 테스트의 분석 대기시간은 150초입니다. 앱은 서버의 완료·오류 응답을 조회하며, 화면 복귀 때 현재 상태를 다시 확인합니다. 외부 검증은 무료 서버에 테스트 프로젝트를 생성하므로 실제 사용자 테스트와 동시에 실행하지 않는 편이 좋습니다.
...
- 업로드 0~15%: XMLHttpRequest의 실제 전송 바이트 비율입니다. 서버 응답을 기다리는 동안은 오디오 준비 15%에 머뭅니다.
- 준비 완료 25%: 서버의 파일 변환·저장이 완료된 뒤 분석 작업을 제출합니다. 대기열에 있으면 분석 순서 대기 메시지를 표시합니다.
- 음정 분석 25~70%: Basic Pitch 0.4의 실제 완료된 오디오 구간 수 / 전체 구간 수입니다. 모델 초기화 중에는 수치가 올라가지 않습니다.
- 멜로디 생성 70~90%: 모델 출력의 음표 변환, 리듬 확인, MIDI 생성 등 실제 작업 경계에서만 갱신합니다.
- 편집 준비 90%, 완료 100%: 결과 파일과 프로젝트가 저장된 뒤에만 완료 응답을 보내고, 화면은 약 0.8초 후 편집기로 이동합니다.
- 숫자의 짧은 보간 애니메이션은 이미 확인된 수치까지만 이동합니다. 시간 경과로 진행률을 만들지 않습니다. 운영체제의 동작 줄이기 설정도 지원합니다.

기존 `GET /api/audio/analyze/{project_id}` 응답은 `status`, `project`와 함께 `job_id`, `stage`, `progress`, `message`를 제공합니다. `GET /api/audio/analyze/{project_id}/status`도 같은 응답을 반환합니다. 기존 프로젝트 ID를 브라우저별 작업 식별자로 재사용하며 POST 재시도는 실행 중인 동일 작업을 중복 실행하지 않습니다. 완료 결과와 실측 `analysis_seconds`는 project.json에 보존됩니다.

분석 중 URL의 `project` 값으로 새로고침 후 같은 작업을 조회합니다. 이미 완료됐다면 편집기를 바로 엽니다. 업로드 응답 전에 탭을 닫은 경우에는 프로젝트 ID가 아직 없으므로 다시 업로드해야 합니다. 실측 분석시간은 작업 시작부터 결과 저장 직전까지이며 업로드·대기열 시간은 제외됩니다. 추정값이 아닌 기본 BPM은 완료 요약에 표시하지 않습니다.
